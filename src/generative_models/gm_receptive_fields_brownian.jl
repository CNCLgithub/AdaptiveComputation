...
@gen static function receptive_fields_brownian_kernel(t::Int,
...
receptive_fields_brownian_chain = Gen.Unfold(receptive_fields_brownian_kernel)

@gen static function gm_receptive_fields_brownian(k::Int,
                                         dynamics_model::AbstractDynamicsModel,
                                         gm::GMMaskParams,
                                         receptive_fields::Vector{AbstractReceptiveField},
                                         prob_threshold::Float64)
    init_state = @trace(sample_init_receptive_fields_state(gm), :init_state)
    states = @trace(receptive_fields_brownian_chain(k, init_state, dynamics_model, receptive_fields, prob_threshold, gm), :kernel)
...
export gm_receptive_fields_brownian
