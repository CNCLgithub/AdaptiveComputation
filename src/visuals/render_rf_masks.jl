...
function get_img(aggregated_masks::Vector{Matrix{Float64}},
    ...
    img = zeros(receptive_fields[end].p2...)
    
    for (i, rf) in enumerate(receptive_fields)
        img[rf.p1[1], rf.p1[2]:rf.p2[2]] .= 0.5
        img[rf.p2[1], rf.p1[2]:rf.p2[2]] .= 0.5
        img[rf.p1[1]:rf.p2[1], rf.p1[2]] .= 0.5
        img[rf.p1[1]:rf.p2[1], rf.p2[2]] .= 0.5
    end

    for (i, rf) in enumerate(receptive_fields)
        img[rf.p1[1]:rf.p2[1], rf.p1[2]:rf.p2[2]] += aggregated_masks[i]
    end
