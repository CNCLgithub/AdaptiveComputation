...
function get_trackers(polygon_structure, targets)
    ...
    return trackers, targets_in_tracked
end

function initialize_constraints(init_objects, trackers, polygon_structure)
...
function run_inference_exp3_polygons(scene; k=nothing)
    # load data
    hgm = MOT.load(HGMParams, joinpath("$(@__DIR__)", "hgm.json"))
    scene_data = MOT.load_scene(scene, dataset_path, hgm)
    ...
    path = joinpath("output", "experiments", "receptive_fields", "$scene", "test")
    try
        mkpath(path)
    catch
    end
    ...
    trackers, targets_in_tracked = get_trackers(polygon_structure, targets)
    ...
    constraints = initialize_constraints(init_objects, trackers, polygon_structure)
    #display(constraints)
    println(k)
    ...
    n_targets = sum(hgm.targets)
    n_distractors_tracked = length(hgm.targets) - n_targets
    td = MOT.get_target_designation(n_targets, n_distractors_tracked,
                                    map_assignment, masks[k],
                                    receptive_fields)
...
#@>> 1:n_scenes foreach(run_inference_exp3_polygons)
run_inference_exp3_polygons(14, k=2)
